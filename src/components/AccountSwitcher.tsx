...
export default function AccountSwitcher({ session }: { session: any }) {
  ...
  useEffect(() => {
    if (isOpen && team.length === 0 && canSwitch && isAdmin) {
      fetch("/api/admin/team")
        .then(res => res.json())
        .then(data => {
          if (Array.isArray(data)) setTeam(data);
          else if (data.team && Array.isArray(data.team)) setTeam(data.team);
          else setTeam([{ id: "error", name: "Помилка завантаження", role: "" }]); // Prevent infinite loading
        })
        .catch(console.error);
    }
  }, [isOpen, team.length, canSwitch, isAdmin]);
              ...
              <div className="max-h-60 overflow-y-auto p-2 space-y-1">
                {team.length === 0 ? (
                  ...
                  team.map((member) => (
                    <button
                      key={member.id}
                      disabled={switchingTo !== null || member.id === session.user?.id || member.id === "error"}
                      ...
                      className={`w-full flex items-center gap-3 p-2 rounded-lg text-left transition ${
                        member.id === session.user?.id 
                          ...
                          : member.id === "error" ? "cursor-default text-red-400" : "hover:bg-white/10"
                      }`}
                    >
                      {member.id !== "error" && (
          ...
          {originalAdminId && (
            <div className="p-2 border-t border-white/5 bg-brand/5">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 p-3 rounded-lg text-brand hover:bg-brand/10 transition text-xs font-bold uppercase tracking-wider"
              >
